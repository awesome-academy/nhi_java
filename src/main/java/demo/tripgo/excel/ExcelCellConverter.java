...
// Đổi giá trị một ô Excel sang kiểu Java của field. Người dùng gõ tay đủ kiểu định dạng nên chỗ
// này nhận nhiều dạng ngày, và mọi lỗi đều kèm tên cột + giá trị thật để họ biết sửa ở đâu.
final class ExcelCellConverter {
    ...
    static Object convert(Cell cell, Class<?> targetType, String header) {
        ...
        // Tiền: dùng BigDecimal, và bỏ dấu phân cách nghìn vì người dùng hay gõ "4.500.000".
        ...
        if (targetType == LocalDate.class) {
            return parseDateTime(cell, value, header).toLocalDate();
        }
    ...
    private static String asText(Cell cell) {
        ...
        return switch (type) {
            ...
            case NUMERIC -> {
                ...
                // Excel lưu mọi số dạng double; số nguyên phải in lại không có ".0" để
                // "4500000" không thành "4500000.0" rồi hỏng khi parse Integer.
    ...
    private static Integer parseInteger(String value, String header) {
        try {
            ...
            throw new ExcelCellException(
                "cột '" + header + "' phải là số nguyên, nhận được '" + value + "'");
        }
    }
    ...
    private static BigDecimal parseDecimal(String value, String header) {
        // "4.500.000" và "4,500,000" đều là cách viết một triệu rưỡi ở hai nơi khác nhau; bỏ hết
        // dấu phân cách rồi parse, vì Excel đã trả số thuần khi ô là kiểu số.
        String cleaned = value.replace(".", "").replace(",", "").replace(" ", "");
        try {
            return new BigDecimal(cleaned);
        } catch (NumberFormatException exception) {
            throw new ExcelCellException(
                "cột '" + header + "' phải là số tiền, nhận được '" + value + "'");
        }
    }
    ...
    private static Boolean parseBoolean(String value, String header) {
        ...
        throw new ExcelCellException(
            "cột '" + header + "' phải là true hoặc false, nhận được '" + value + "'");
    }
    ...
    private static LocalDateTime parseDateTime(Cell cell, String value, String header) {
        ...
        throw new ExcelCellException("cột '" + header
            + "' phải là ngày (vd 2026-01-31 hoặc 31/01/2026), nhận được '" + value + "'");
    }
    ...
    private static Object parseEnum(String value, Class<?> enumType, String header) {
        ...
        String allowed = Arrays.stream(enumType.getEnumConstants())
            ...
            .toList()
            .toString();
        throw new ExcelCellException("cột '" + header + "' có giá trị '" + value
            + "' không hợp lệ. Cho phép: " + allowed);
    }
}
