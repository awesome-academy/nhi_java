...
// Đọc/ghi Excel cho BẤT KỲ lớp nào có field gắn @ExcelColumn, nhờ reflection. Thêm một loại dữ
// liệu mới (đơn đặt, tour, điểm đến...) chỉ cần tạo lớp và gắn annotation — không phải viết thêm
// một dòng code đọc/ghi nào.
@Component
public class ExcelMapper {

    // Chặn file quá lớn làm hết bộ nhớ: POI đọc cả workbook vào RAM.
    public static final int MAX_DATA_ROWS = 5_000;

    private static final String DATE_PATTERN = "dd/mm/yyyy";

    // Quét annotation khá tốn; mỗi lớp chỉ quét một lần rồi dùng lại.
    ...
    private static List<ExcelField> scan(Class<?> type) {
        ...
        // Duyệt cả lớp cha để lớp con kế thừa được cột chung.
    ...
    public <T> ExcelReadResult<T> read(InputStream input, Class<T> type) {
        ...
        try (Workbook workbook = WorkbookFactory.create(input)) {
            ...
            Map<ExcelField, Integer> layout = matchColumns(fields, readHeader(headerRow));
            ...
            for (int index = headerRow.getRowNum() + 1; index <= sheet.getLastRowNum(); index++) {
                ...
                // Số dòng hiển thị = chỉ số + 1 để khớp với thanh số dòng trong Excel.
            ...
            throw new ExcelParseException(
                "File tải lên không phải file Excel hợp lệ hoặc đã bị hỏng", exception);
        }
    }
    ...
    // Khớp cột theo TIÊU ĐỀ chứ không theo vị trí: người dùng đảo cột hay chèn thêm cột ghi chú
    // vẫn nhập được, và thiếu cột bắt buộc thì báo rõ thiếu cột nào.
    private Map<ExcelField, Integer> matchColumns(
        List<ExcelField> fields, Map<String, Integer> columnIndexes
    ) {
        ...
        if (!missingRequired.isEmpty()) {
            throw new ExcelParseException(
                "File thiếu cột bắt buộc: " + String.join(", ", missingRequired));
        }
        return layout;
    }
            ...
            if (value == null) {
                if (field.required()) {
                    throw new ExcelCellException(
                        "thiếu giá trị ở cột bắt buộc '" + field.header() + "'");
                }
                continue;
            }
    ...
    private <T> T instantiate(Class<T> type) {
        try {
            ...
            throw new IllegalStateException(type.getSimpleName()
                + " cần có constructor rỗng để ExcelMapper tạo đối tượng khi đọc file", exception);
        }
    }

    // Dòng rỗng bị bỏ qua thay vì báo lỗi "thiếu giá trị". Phải tính cả ô CHUỖI RỖNG: người dùng
    // gõ rồi xoá để lại ô kiểu STRING với giá trị "", không phải ô BLANK — file thật đầy dòng
    // kiểu này ở cuối bảng.
    ...
    public <T> byte[] write(List<T> rows, Class<T> type, String sheetName) {
        ...
        try (XSSFWorkbook workbook = new XSSFWorkbook();
            ...
            // Khoá dòng tiêu đề để cuộn danh sách dài vẫn thấy tên cột.
            ...
            throw new ExcelParseException(
                "Không ghi được file Excel: " + exception.getMessage(), exception);
        }
    }
    ...
    private void fill(Cell cell, Object value, CellStyle dateStyle) {
        switch (value) {
            ...
            // phạm vi này thì an toàn, và để dạng số thì Excel mới tính tổng được.
